...
function renderFunnel(map){ const host = $('#funnel-host'); if(!host) return; const origins = Object.keys(map).sort(); if(origins.length===0){ host.innerHTML = '<p class="muted">イベントがありません。</p>'; return; }
  ...
  for(const o of origins){ const m = map[o]; const c = m.click_card||0; const v = m.view_schedule||0; const s = m.submit_request||0; const cvr = v>0 ? Math.round((s/v)*100) : 0; html += `<tr><td>${escapeHtml(o)}</td><td>${c}</td><td>${v}</td><td>${s}</td><td>${cvr}%</td></tr>`; }
  html += '</tbody></table>';
  host.innerHTML = html;
}
