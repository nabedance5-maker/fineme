...
export function costSummary(logs) {
  ...
  const byAxisMap = {}; // 軸ごとに合算する（同じカテゴリを違う名前で複数登録しても1行にまとめる）
  ...
  for (const log of logs || []) {
    ...
    if (!byAxisMap[log.axis]) {
      byAxisMap[log.axis] = { axis: log.axis, customIcon: log.custom_icon, monthly: 0, estimated: false, count: 0 };
    }
    byAxisMap[log.axis].monthly += m;
    byAxisMap[log.axis].count++;
    if (isEstimated) byAxisMap[log.axis].estimated = true; // 1件でも目安頻度で計算していれば「約」を付ける
  }

  const byAxis = Object.values(byAxisMap).sort((a, b) => b.monthly - a.monthly);
