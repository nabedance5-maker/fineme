...
const MONTHLY_NUDGE = {
  crew: {
    diagnosis: 'そういやお頭、海図の方はどうなってやすか？\n▸ https://www.fineme.me/diagnosis',
    mirror:    'そういやお頭、今の面構えは測っときやしたか？\n▸ https://www.fineme.me/mirror',
  },
  navigator: {
    diagnosis: 'ところでキャプテン、海図はもうお持ちですか。\n▸ https://www.fineme.me/diagnosis',
    mirror:    'ところでキャプテン、現在地はまだ測っていません。\n▸ https://www.fineme.me/mirror',
  },
  plain: {
    diagnosis: 'Me Scan（無料・約3分）で、どこから手をつけると効くかが分かります。\nhttps://www.fineme.me/diagnosis',
    mirror:    'Mirror で、いまの現在地を写真1枚から測れます。\nhttps://www.fineme.me/mirror',
  },
};
...
export function buildLogMessage(booking, reminder, resolveAxis, opts = {}) {
  ...
  const nudge = MONTHLY_NUDGE[v.id]?.[opts.monthlyNudge];
  if (nudge) {
    lines.push('');
    lines.push('- - -');
    lines.push(nudge);
