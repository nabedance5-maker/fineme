...
// { label, type: 'uri', uri } の形も受け付ける（外部ページを開くだけのボタン。
// 体験談投稿への導線等・でお要望2026-09-09）。type省略時はpostback扱い。
  ...
  if (quickReplyItems?.length) {
    message.quickReply = {
      items: quickReplyItems.map(q => ({
        type: 'action',
        action: q.type === 'uri'
          ? { type: 'uri', label: q.label, uri: q.uri }
          : { type: 'postback', label: q.label, data: q.data, displayText: q.displayText || q.label },
      })),
    };
  }
