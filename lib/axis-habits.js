...
// ─── 選択肢の重み（でお指摘 2026-08-14：選択肢は同等じゃない。化粧水はやっててほしいが
// ピーリングは必須ではない）───
// 「これだけは押さえていてほしい」基本行動。inferPathTypeの3値バケット（virgin/blind/doing）
// はitems.lengthの有無・プロ関与の有無しか見ないため、「ピーリングだけやっている人」と
// 「化粧水・乳液・クレンジング・日焼け止めを揃えている人」が同じスコアになってしまう。
// この基本セットのカバー率でblind（自己流）内のスコアを2.0〜3.5の範囲で細分化する。
// 軸によっては「これが基本」と言えるものが無い（bodyの種目選びは好み次第、等）ため、
// 定義した軸だけ効く。未定義の軸は従来通り一律スコア。
export const ESSENTIAL_ITEMS = {
  fashion: ['fit_check'],
  hair:    ['shampoo_market', 'shampoo_salon'],
  skin:    ['lotion', 'cream', 'cleansing', 'sunscreen'],
  teeth:   ['electric_toothbrush', 'floss', 'checkup'],
};

// 基本セットのカバー率（0〜1）。軸に基本セット定義が無ければnull
export function essentialCoverageRatio(axisId, items) {
  const essentials = ESSENTIAL_ITEMS[axisId];
  if (!essentials || !essentials.length) return null;
  const covered = essentials.filter(k => (items || []).includes(k)).length;
  return covered / essentials.length;
}

// ─── 変えたい度（でお指摘 2026-08-14：具体行動の質問に加えて、カテゴリごとの
// 変えたい度も聞いた方がいい）───
// 「今の状態を自己評価させる」質問（自己申告のラダー）とは別物：これは自己評価ではなく
// 本人の意思・目標そのものなので、自己申告特有の不正確さの問題が起きない。
// 回答があれば理想値（ideal）をこの値で直接上書きする。未回答なら現在値ベースの自動算出
// （現状維持）にフォールバックする
export const WANT_CHANGE_OPTIONS = [
  { v: '1', t: '今のままでいい' },
  { v: '2', t: '少し変えたい' },
  { v: '3', t: 'まあまあ変えたい' },
  { v: '4', t: 'けっこう変えたい' },
