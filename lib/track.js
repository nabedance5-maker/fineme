...
//
// でお指摘 2026-08-06：未ログインで新しくMe Scanを受けた端末で、その後
// 「既にMe Scan済み・Map生成済みの別アカウント」にログインすると、確認なしで
// アカウントの診断結果が上書きされ、New Me Mapが意図せず作り変わってしまう事故があった。
// → ログイン先アカウントに既に診断結果がある場合は、ローカルのゲスト診断で
// 上書きせず、サーバ側を正としてローカルをサーバのデータで揃える（黙って破棄）。
export async function syncLocalDiagnosisToServer(accessToken) {
  ...
  for (const id of ['fineme', 'belle']) {
    ...
    try {
      ...
      // ログイン先アカウントに既存の診断結果があるか先に確認する
      let existing = null;
      try {
        const existingRes = await fetch(`/api/me/diagnosis?track=${id}`, {
      ...
      if (existing?.transform_vectors) {
        // 既存アカウントの本物のデータを優先し、端末のゲスト診断は破棄。
        // ローカルもサーバの内容に揃えておく（mypage/navi はローカル参照のため）
