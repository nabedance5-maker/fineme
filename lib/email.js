...
// ── LINEから直接送られた予約リクエスト（New Me Log経由・来店日未定）：掲載者に通知 ──
// フォーム入力を挟まずLINEのボタン1つで送られてくるため、来店日はまだ決まっていない。
// 続きはLINEのトーク画面で店舗と直接すり合わせてもらう前提（でお要望2026-09-09）。
export async function sendLineBookingRequestEmail({ providerEmail, providerName, userName, note }) {
  if (!providerEmail) return;
  await send({
    to: providerEmail,
    subject: '【Fineme】LINEから予約リクエストが届きました',
    html: `
      <h2 style="color:#111">LINEからの予約リクエスト</h2>
      <p>${providerName || '掲載者'} 様</p>
      <p>New Me LogのLINEから、予約リクエストが届きました。来店日はまだ決まっていません。LINEのトーク画面で直接ご相談ください。</p>
