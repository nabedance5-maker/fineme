 ...
 * 日時は決めない（pendingで作成し、店舗からの連絡を待つ）。ゲスト（未ログイン）は
 * 連絡先・本人確認ができないため未対応——呼び出し側でログイン必須にする。
 * @param {string} id      ログのid
 * @param {string} message 任意の一言メッセージ
 * @returns {Promise<{ok:true, reservation:object, providerName:string}>}
 */
export async function requestBooking(id, message) {
  const token = getAccessToken();
