...
/**
 * Fineme掲載店舗（provider_slug付き）へ、Logのカードから直接予約をリクエストする。
 * あくまで「希望」であり確定ではない（pendingで作成し、店舗からの連絡を待つ）。
 * ゲスト（未ログイン）は連絡先・本人確認ができないため未対応——呼び出し側でログイン必須にする。
 * @param {string} id
 * @param {{preferredDate:string, preferredTime?:string, menuName?:string, message?:string}} opts
 *   preferredDate は必須（でお指摘2026-09-09：日時の希望を送れないと使えない）
 * @returns {Promise<{ok:true, reservation:object, providerName:string}>}
 */
export async function requestBooking(id, { preferredDate, preferredTime, menuName, message } = {}) {
  const token = getAccessToken();
  if (!token) throw new Error('ログインが必要です');
  const r = await fetch(`/api/me/service-logs/${id}/book-request`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
    body: JSON.stringify({ preferred_date: preferredDate, preferred_time: preferredTime || '', menu_name: menuName || '', message: message || '' }),
  });
  if (!r.ok) throw new Error((await r.json().catch(() => ({}))).error || 'リクエストに失敗しました');
  return r.json();
}
