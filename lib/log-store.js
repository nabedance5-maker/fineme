 ...
 * @param {string} id
 * @param {{preferredDate:string, preferredTime?:string, message?:string}} opts
 *   preferredDate は必須（でお指摘2026-09-09：日時の希望を送れないと使えない）
 * @returns {Promise<{ok:true, reservation:object, providerName:string}>}
 */
export async function requestBooking(id, { preferredDate, preferredTime, message } = {}) {
  const token = getAccessToken();
  if (!token) throw new Error('ログインが必要です');
  const r = await fetch(`/api/me/service-logs/${id}/book-request`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
    body: JSON.stringify({ preferred_date: preferredDate, preferred_time: preferredTime || '', message: message || '' }),
  });
