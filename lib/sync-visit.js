...
export async function syncVisitToLog(db, { userId, providerSlug, visitDate, cost = null }) {
  ...
  // サーバー（Vercel）はUTCで動くため、new Date().toISOString()をそのまま使うと
  // JST 0:00〜9:00の間は「日本時間の前日」の日付になってしまう（でお報告2026-09-15：
  // 予約カレンダーの日付ズレと同根の不具合。日付未指定のチェックイン等で来店日が
  // 1日前として記録されてしまうのを防ぐ。Financeは日本国内向けのため常にJSTで判定）。
  const date = visitDate || new Intl.DateTimeFormat('sv-SE', { timeZone: 'Asia/Tokyo' }).format(new Date());
  try {
