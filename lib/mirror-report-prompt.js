...
【出力は以下のJSON形式のみ（コードブロックなし、JSONだけ）】
{
  ...
  "scores": {
    ...
    "eyes": 0-100の数値またはnull,
    "eyebrows": 0-100の数値またはnull,
    "nose": 0-100の数値またはnull,
    "mouth": 0-100の数値またはnull,
    "faceline": 0-100の数値またはnull,
    "symmetry": 0-100の数値またはnull,
...
- 顔が写っていない/判別できない場合、eyes/eyebrows/nose/mouth/faceline/symmetry/faceの評価に必要な各項目はnullにする

【scores・score_weightsの出し方（最重要）】
- scoresは各カテゴリの視覚的完成度を0-100で評価する（数値が高いほど「すでに整っている」＝低いほど「伸びしろが大きい」）。
- score_weightsは「この写真の第一印象を左右している度合い」を0-100で評価する。全カテゴリが均等ではない。
  例：顔がアップの写真ならface_balance/eyes/skin/hairの重みを高く、全身写真ならbody_shaping/fashion/postureの重みを高くする。
  写真から見て取れる範囲で、実際にその写真の印象を最も強く決めている要素を高く評価すること。適当な均等割りにしない。
...
- スコアの誠実性ルール（最重要）：スコアは写真から実際に観察できる事実だけに基づいて、誠実につけること。
  ...
  根拠があれば普通に使ってよい。無理に中間（40〜85点等）に寄せない。本当に整っている写真には高いスコアを、
  本当に伸びしろが大きい写真には低いスコアを、恐れずそのままつける。数字を歪めることは、このサービスの
  信頼性を損ない、結果としてユーザーの役に立たない
- potential_level「高」の軸に対応するカテゴリはscoresを低めに、potential_level「低」の軸は高めにして、
  axesの記述内容とscoresの数値が矛盾しないようにする（これは数値の誠実性の話であり、上記の作為的調整とは別）
- 「傷つけない」はスコアの数値ではなく、summary・detail・first_impression等の文章の伝え方（分析の原則にある
