...
function categoryPhoto(cat) {
  const map = {
    ...
    colordiagnosis: 'https://images.unsplash.com/photo-1512496015851-a90fb38ba796?q=80&w=800&auto=format&fit=crop',
    bonediagnosis: 'https://images.unsplash.com/photo-1548690312-e3b507d8c110?q=80&w=800&auto=format&fit=crop',
...
function SearchContent() {
  ...
  const [area, setArea] = useState(() => {
    ...
    // 旧形式の英語コードをサポート
    const legacyMap = { tokyo: '東京', osaka: '大阪', kanagawa: '神奈川', aichi: '愛知', fukuoka: '福岡', saitama: '埼玉', chiba: '千葉', kyoto: '京都', hyogo: '兵庫', hokkaido: '北海道' };
    if (legacyMap[r]) return legacyMap[r];
    if (r) return r; // 日本語のまま渡された場合はそのまま使用
