...
export async function GET(request) {
  ...
  const [{ data: arts }, { data: xposts }] = await Promise.all([
    sb.from('features').select('slug,title,description,category,published_at').eq('status', 'published').order('published_at', { ascending: false }).limit(12),
  ...
  const artList = articles.map((a, i) => `${i}: [${a.category}] ${a.title}｜${(a.description || '').slice(0, 50)}`).join('\n');
  const xList = (xposts || []).map((p, i) => `${i}: ${(p.text || '').replace(/\n/g, ' ').slice(0, 60)}`).join('\n');

  const prompt = `あなたはFineme（恋愛に悩む男性向け外見改善×AI事業／北極星=年商10億・年収1億／今は集客フェーズ）のグロースを預かる辛口CMO。
...
観点：①タイトル/切り口が定型・金太郎飴になっていないか（同じ骨格の連発）②内容が差別化され読者の得になるか③10億ゴール(集客→販売)に効くか④検索/SNSで実際に勝てるか。
...
次のJSONのみ出力（コードブロック無し・日本語）：
{
  ...
  for (const r of (critique.rewriteTitles || []).slice(0, 2)) {
    ...
    if (!error) { try { revalidatePath(`/feature/${a.slug}`); } catch {} applied.push({ slug: a.slug, from: a.title, to: r.newTitle, reason: r.reason }); }
  }
