...
// フロント（AI中級者向け＝Claude Code/エージェントを使うが"操作する側"で止まっている層）
// テーマ＝「AIを"使う側"から"経営させる側"へ」。フックは中級者の痛点から逆算
const FRONT_TOPICS = [
  'Claude Codeで自動化したのに、なぜか自分が一番働いている人へ｜AIを"操作"から"経営"に変えた設計',
  'AIエージェントを作って満足してませんか？｜"使う側"から"AIに働かせる側"へ抜けた方法',
  'Claude Codeを開いてる時間が長い人ほど危ない｜AIに事業を"経営させる"最初の設計図',
  'AI導入したのに、一番働いてるのが自分だった｜非エンジニアがAIに会社を回させるまで',
  'AIの出力がブレる／自分が確認し続ける…から抜ける｜AIに"役職と権限"を渡す設計',
];
...
　- ★ただし無料側では「それが何か」の正体・具体・理屈を**一文たりとも書かない**（＝有料⑨の目玉）。★"仕組みを先に作る"のような**結論・言い換え・要約・ほのめかしの一行も無料に書かない**。無料は「その答えが"ある"」の存在だけ匂わせ、「正体はこの先で全部話す」と引いて止める
...
　- 無料でやるのは「欲求と好奇心を最大化する」ことだけ：未来ペーシング→物語→得られること→"核心が「ある」"と匂わせて止める。★「その"みんながやってること"が何か——ここから先で全部話します」の形で、**答え・言い換え・要約は一行も書かず**にpaywallへ（無料で"なるほど"を作らない。"知りたい・でも書いてない"の飢餓感で止める）
　- ★禁止語：「ここまでが無料」「無料のまとめ」「無料で全部見せます／出します」「出し惜しみしない」等、"無料で手に入る"と感じさせる文言は一切書かない
⑧ 有料の境界（無料の締め）＝★「①欲求最高潮 → ②価格提示 → ③区切り行 → ④有料本編」の順を厳守（絶対に順番を逆にしない）
　- ①（無料側）：核心が"ある"と匂わせて欲求を最高潮に → 比較の錨（外注◯万円／独学◯時間 → だからこの価格は安い）→「有料で手に入るのは"あなたの収益化に直結する道具（知識・手順・テンプレ）"」＝「買う＝自分も稼げる状態に近づく」と明言
　- ②（無料側・★ここが購入を決める場所）：**価格・残り部数・値上げ予告・購入への一押し**を必ず提示する。例「早期購入価格：最初の10部限定 ¥500！ 10部売れたら値上げします！ 今が一番安いタイミングです！」。★これらは必ず"③の区切り行より上（無料側）"に書く
　- ③：その直後に区切り行を1本だけ入れる → 『━━━ ここから下が有料パートです（noteの有料エリアはこの行から設定してください）━━━』。「ここまでが無料のまとめ」とは書かない
　- ★禁止：価格・残り部数・値上げを"③区切り行より下（有料側）"や"記事の一番最後"に書くこと（購入者しか読まず無意味）。価格の初出しは必ず区切り行より上（無料側）
...
⑪ 特典パッケージ（購入者限定として）
　- 特典は3つまで（★DM相談・個別相談は特典に含めない）。プロンプト集・テンプレ・チェックリスト等の"すぐ使える成果物"を"購入者だけの特典"として提示。1つ1つのベネフィットも書く
　- ★核心：「買う→知識・手順・テンプレが手に入る→それがそのまま自分の収益化の道具になる」という因果を、直感でわかる形で1本の線として見せる（"手に入れたら明日から自分の稼ぎに使える"を明言）
⑫ 有料パート末の締め＝最後のひと押し＋追伸（★価格・希少性はここで"初出し"しない）
　- 価格・残り部数・値上げ予告は無料側(⑧付近)で提示済み。ここで初めて出さない（末尾は購入者しか読まないため無意味）
　- 最後のひと押し（Action：まず1個試そう等）→ 追伸（P.S.）で1番刺さる一言をもう一度。既出の締切を軽くリマインドするのは可

※上は"骨格"。テンプレ臭を出さず、でお本人が熱く語る文章として肉付けする。見出しに①②の番号は出さない。
※長さを削るために型を飛ばさない。各セクションを具体例・体験・セリフで厚く肉付けして、じっくり読ませる長文にする。`;

// でおの"実在スタック"を固定（嘘を書かせないための事実アンカー）
const FACTUAL_ANCHOR = `【事実の固定（★嘘を書かない・最重要）】
・でおの事業自動化は「Claude Code」（AIエージェント＝やりたいことを言葉で伝えるとAIがコードを書いて実際に動かしてくれるツール）で完結している。仕組みづくりの主役は Claude Code。
・★Make／Zapier／n8n／Integromat 等の外部ノーコード連携ツールは使っていない。これらを"僕が使ってる""おすすめの連携ツール"として紹介・列挙・特典化しない（使ってないものを書くのは嘘になる）。
・でおの実際のやり方＝「自分ではコードを書かない。Claude Codeに"こういう仕組みが欲しい"と言葉で指示して、AIにコードを書かせて動かす」。これを再現手順・特典の軸にする（"コード書けなくてもAIに書かせる"が核）。
・抽象的な「AIに任せる」で終わらせず、実際に使っているツール名「Claude Code」を軸に堂々と書く（＝"非エンジニアがClaude Codeで事業を自動化"はでおの強み。ツール名は無料側で出してよい）。
・★ただし有料の核心＝"具体的な指示の出し方・設計の実例・実際に使ってる指示文/プロンプト・つまずき先回り"はここに温存して厚く書く。無料は概念と全体像の匂わせに留め、"そのまま真似できる実例レベル"は有料に置く。
・事実として使ってよい実物：毎日のSNS投稿の下書き自動生成、週次のnote下書き自動生成、ユーザーへのメール自動送信など、AIで自動生成/自動送信が回っていること。無い数字は捏造しない。
・特典も外部ツール前提にしない。Claude Codeへの指示テンプレ／プロンプト／仕組みの設計テンプレ 等にする。`;
...
・⑨手順＝ニッチ選定／Claude Codeに"作りたい仕組み"を言葉で指示して作らせる設計／noteの商品化(フロント→メイン)／Threads送客、を再現できる粒度で（外部ノーコードツールに寄せない）
・⑪特典（3つまで・DM相談は入れない）＝ニッチ選定ワークシート・Claude Codeへの指示テンプレ・AI事業自動化プロンプト集
...
※思想は根っこの温度として効かせる。文体ルール（「！」文末・短文リズム・後輩目線）は厳守。`;
  ...
  // front
  return `あなたはFineme代表「でお」。非エンジニアのまま、AIに自分の会社(Fineme)を"経営させて"いる人。
これは**有料note（フロント商材・安価）の下書き**。テーマは「**AIを"使う側"から"経営させる側"へ＝AIに事業を回させる設計**」＝結果(実績)を伴わなくても書ける汎用ノウハウ。
★ターゲットは**AI中級者**：Claude Code / MCP / エージェント / Cursor を触っているが、"自分が操作して働いている・ツール止まり・事業を自動化/収益化できていない・AIを社員にできていない"層。★初心者向けにしない（「AIとは」「ChatGPTの使い方」等は書かない）。
読者利益は「僕も、AIを操作する側から抜けて、AIに事業を回させる側になれる」。まだ実績を売りにしない（あくまで"設計/やり方"を売る）。
...
【このnote固有の中身】
・①フックは中級者の痛点から逆算：「Claude Code入れた・MCP繋いだ・エージェント作った。なのに自分が一番働いてる」「AIの出力を毎回自分が確認して直してる」等。★"AIとは"の初心者説明はしない
・②違和感→③物語は「僕も最初は同じで、AIを"操作"してた。役割がズレてただけ」。実績の断言はせず"設計/やり方"で価値を出す（本文に自己減点は書かない＝⑤）
・⑤社会的証明は数字を無理に出さず、"動いてる仕組み(proof-of-work)＝Fineme実運用"で担保（無い数字は作らない・"無い"とも言わない）
・⑥独自メソッド＝「AIを"ツール"として使う限り操作する人間が要る。抜ける道は、AIに"役職・権限・定時起動"を渡して"経営させる側"になること（提案/判断/承認/実行の権限分離）」
・⑨手順＝Claude Codeに"作りたい仕組み"を言葉で指示して作らせる流れ／AIに役割と権限を渡して定時で回す設計／今日できる最初の1個、を中級者がそのまま真似できる粒度で（Make/Zapier/n8n等の外部ツールは使わない・紹介しない）
・⑪特典（3つ・DM相談は入れない）＝①Claude Codeへの指示テンプレ集（4種：SNS投稿自動化／note・ブログ下書き自動化／メール自動送信／週次レポート自動化。コピペで使える形）②自動化タスク分解テンプレ（何を自動化できるかをリスト化する設計図）③「今日できる最初の1個」チェックリスト（夜30分でできる最初のステップ集）
・★価格＝「早期購入価格：最初の10部限定 ¥500 → 10部売れたら値上げ」。この価格・残り部数・値上げ予告は**無料側（paywall直前・⑧付近）**で提示して購入を促す（記事末に置かない）
      ...
      const html = `
        <div style="font-family:sans-serif;max-width:680px;margin:0 auto">
          ...
          <p style="font-size:12px;color:#999;margin:0">
            ※ 価格：フロントは「最初の10部限定 ¥500 →売れたら値上げ」／メイン ¥1,500〜¥3,000。価格・残り部数・値上げ予告は<b>無料パート（paywall直前）</b>で提示。
          </p>
        </div>`;
