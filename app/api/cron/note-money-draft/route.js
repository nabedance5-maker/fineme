...
⑤ 社会的証明（proof-of-work を最優先）
　- 部数・view・購入者の声があれば出す（実数は必ず「◯」プレースホルダ・無い数字は作らない）
　- ★重要：実績数字がまだ無くても「実績はまだ無い」と言い切って終わらせない。代わりに「今この瞬間に動いてる仕組みそのものが証拠」＝proof-of-workで信頼を作る（例：この記事の配信・僕のSNS投稿・サービス運用が、実際にAIで自動で回っている＝それが生きた実証）。正直さは保ちつつ、証明の空白を"動いてる現物"で埋める
⑥ 独自メソッドの宣言（Solution）
　- 「みんなは◯◯してるけど、逆」の逆張り・独自の切り口で"ありきたりじゃない"を出す。なぜ効くのかの理屈も添える
⑦ 無料パートで"考え方"を出し切る（役割分担が重要）
　- 無料は「なぜそうなるかの考え方＋最初の1歩」まで出し切って「タメになった」と思わせる
　- ★ただし"実際に動かす全手順・プロンプト全文・テンプレ・配線の具体"は有料に取っておく。無料で完結させない（考え方は無料／道具と手順は有料、で明確にギャップを作る）
　- 無料の終わりに軽い中間CTA（「続きでは実際に使ってる道具と手順を全部渡します」等）で有料への橋を1つ架ける
⑧ 有料の境界（1回だけ）＋価格アンカリング
　- 「ここから先は」的な区切りを1箇所。ここまでが無料・この先が"道具と全手順"とハッキリ分ける
