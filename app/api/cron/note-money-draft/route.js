...
【実数の扱い（最重要）】売れた部数・金額・view・フォロワー等の具体数字は、まだ僕が実データを渡していない。だから断言せず「◯部」「◯円」「◯view」のように必ず ◯ のプレースホルダで書く（あとで僕が実数を入れる）。ありもしない数字を勝手に断言しない！

【構成】フック（あなたもニッチ×AIで稼げる）→ 僕のゼロ→成功物語＋実数(◯) → なぜAI×外見磨き(ニッチ)が勝てるのか → 再現手順（ニッチ選定/AI活用/note構成/Threads送客）→ 実績(◯) → 特典（プロンプト集/テンプレ）→ CTA
【CTA】記事末に、外見の悩みがある読者向けに Fineme（${BASE_URL}/lp/mirror）へのソフトな導線を1つ。押し売りしない！
【長さ】3000〜4500字。見出し（##）を4〜6個。有料部分の見出しには（ここから有料）等の目印を1箇所入れてよい。
【出力形式】1行目「# {タイトル}」→空行→本文（Markdown ##, 段落, **太字**）。前置き不要・記事のみ出力。
...
【構成】フック（AIで自動化したいけど何から？の痛み/憧れ）→ 僕の実像（顔もセンスもゼロ→今はAIに事業を任せてる）→ AIで事業を自動化する考え方（小さく始める/人の作業を置き換える/毎日勝手に進む仕組みを足す）→ 具体手順（AIに何を任せるか・今日できる小さな自動化1個）→ 特典（すぐ使えるプロンプト/分解テンプレ）→ 次への布石
【CTA】記事末に、Threadsフォロー＋「この考え方で"誰もやってないニッチ"で実際に事業を作った話は近日メインnoteで公開します」の予告を自然に1つ。押し売りしない！
【長さ】2200〜3200字。見出し（##）を4〜6個。
【出力形式】1行目「# {タイトル}」→空行→本文（Markdown ##, 段落, **太字**）。前置き不要・記事のみ出力。
...
async function generateNote(mode, topic) {
  ...
  const msg = await client.messages.create({
    ...
    temperature: 0.9,
    system,
    messages: [{ role: 'user', content: `今回の有料noteテーマ：「${topic}」\nこのテーマで、最後まで読まれて"自分もやってみよう"につながる有料note本文を1本書いてください。` }],
  });
...
export async function GET(request) {
  ...
  const mode = new URL(request.url).searchParams.get('mode') === 'main' ? 'main' : 'front';
