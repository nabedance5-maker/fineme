...
export default function SchedulePage() {
  ...
  useEffect(() => {
    ...
    style.textContent = `
      ...
      .time-cell{padding:12px;border:1px solid var(--color-border,#e2e8f0);border-radius:8px;background:#fff;display:flex;flex-direction:column;align-items:center;justify-content:center;cursor:pointer}
      .time-cell .time{font-weight:700;margin-bottom:6px}
      .time-cell .symbol{font-size:20px;line-height:1}
      .time-cell.available{background:linear-gradient(90deg,var(--primary,#0b2640),#0e3760);color:#fff}
      .time-cell.unavailable{opacity:0.6;background:#fff;color:var(--color-muted,#94a3b8);cursor:not-allowed}
      .calendar-matrix{overflow:auto}
      .matrix-table{border-collapse:collapse;width:100%;min-width:720px}
      .matrix-table th,.matrix-table td{border:1px solid var(--color-border,#e2e8f0);padding:8px;text-align:center;vertical-align:middle}
      .matrix-table th{background:#f8fafc;font-weight:700}
      .time-label{width:80px;background:#fff;border-right:1px solid var(--color-border,#e2e8f0);font-weight:600}
      .matrix-cell{height:56px;padding:6px;cursor:pointer}
      .matrix-cell.available{background:linear-gradient(90deg,var(--primary,#0b2640),#0e3760);color:#fff}
      .matrix-cell.unavailable{background:#fff;color:var(--color-muted,#94a3b8);opacity:.6;cursor:not-allowed}
    ...
    function renderSlotsList() {
      ...
      if (calView === 'week') {
        ...
        host.querySelectorAll('.matrix-cell.available').forEach(cell => cell.addEventListener('click', () => {
          ...
          openReqModal({ slotId: id, start, end, date: cellDate });
        }));
        return;
      }
    ...
    function openReqModal(info) {
      ...
      const reqServiceId = qs('#req-serviceId');
      if (reqServiceId) reqServiceId.value = sid;
      const customFields = qs('#req-custom-fields');
      if (info && info.custom) {
        const reqSlotId = qs('#req-slotId'); if (reqSlotId) reqSlotId.value = '';
        if (customFields) customFields.hidden = false;
        const summary = qs('#req-slot-summary'); if (summary) summary.textContent = `別の時間をリクエスト: ${info.date || ''}`;
        ...
        const reqSlotId = qs('#req-slotId'); if (reqSlotId) reqSlotId.value = info.slotId;
        const summary = qs('#req-slot-summary'); if (summary) summary.textContent = `選択枠: ${info.date} ${info.start} - ${info.end}`;
        if (customFields) customFields.hidden = true;
      }
      // Prefill from user session
      try {
    ...
    function showAuthPrompt(info) {
      ...
      try {
        ...
        if (loginLink) loginLink.setAttribute('href', '/user/login?next=' + next);
        if (regLink) regLink.setAttribute('href', '/user/login?next=' + next + '#register');
    ...
    function onSubmitReq(e) {
      ...
      let userInfo = null;
      try { userInfo = getUserSession(); } catch {}
      ...
      // ── Supabase API に同期（メール通知もここで発火） ──
      const providers = (() => { try { return JSON.parse(localStorage.getItem('glowup:providers') || '[]'); } catch { return []; } })();
      const providerObj = providers.find(p => p.id === svc.providerId || p.profile?.id === svc.providerId);
      const providerEmail = providerObj?.profile?.email || providerObj?.email || '';
      const providerName = providerObj?.profile?.name || providerObj?.name || '';
      fetch('/api/reservations', {
        ...
        body: JSON.stringify({
          ...
          note,
          provider_id: String(svc.providerId),
          service_id: String(sid),
          service_name: svc.name || '',
          price: svc.price || 0,
          commission_rate: svc.commissionRate || 0.08,
          reserved_date: slot ? slot.date : customDate,
          start_time: slot ? slot.start : customStart,
          end_time: slot ? slot.end : customEnd,
          origin: getParam('origin') || 'direct',
          provider_email: providerEmail,
          provider_name: providerName,
        }),
      }).catch(e => console.warn('[reservation API]', e));

      // ローカル通知（後方互換）
      try {
        // addNotification is a legacy module import; use dynamic localStorage approach
        const notifKey = 'fineme:notifications';
        const notifs = (() => { try { return JSON.parse(localStorage.getItem(notifKey) || '[]'); } catch { return []; } })();
        notifs.push({ toType: 'provider', toId: svc.providerId, title: '新しい予約リクエスト', body: `${svc.name} に ${req.date} ${req.start} のリクエストが届きました（${name}）`, data: { requestId: req.id }, ts: new Date().toISOString() });
        notifs.push({ toType: 'user', toId: userInfo?.id || null, title: '予約リクエストを送信しました', body: `${svc.name} へ ${req.date} ${req.start} のリクエストを送信しました。`, data: { requestId: req.id }, ts: new Date().toISOString() });
    ...
    (function init() {
      ...
      if (svc) {
        const titleEl = qs('#title'); if (titleEl) titleEl.textContent = `スケジュール選択 - ${svc.name}`;
        const subtitleEl = qs('#subtitle'); if (subtitleEl) subtitleEl.textContent = `カテゴリ: ${svc.category} / 地域: ${svc.region}`;
        ...
        // match rendering (optional, no external module dependency)
        ...
        if (sel) {
          ...
          filtered.forEach(s => {
            ...
            opt.value = s.id;
            opt.textContent = `${s.name}（¥${Number((s.price != null ? s.price : s.priceMin) || 0).toLocaleString()}）`;
            sel.appendChild(opt);
          });
          sel.addEventListener('change', () => {
            const val = sel.value;
            const u = new URL(location.href);
            if (val) u.searchParams.set('serviceId', val); else u.searchParams.delete('serviceId');
            history.replaceState({}, '', u.toString());
            const svc2 = loadServices().find(x => x.id === val);
            if (svc2) {
              const titleEl = qs('#title'); if (titleEl) titleEl.textContent = `スケジュール選択 - ${svc2.name}`;
              const subtitleEl = qs('#subtitle'); if (subtitleEl) subtitleEl.textContent = `カテゴリ: ${svc2.category} / 地域: ${svc2.region}`;
            }
      ...
      qs('#today-btn')?.addEventListener('click', () => {
        const n = new Date(); viewYear = n.getFullYear(); viewMonth = n.getMonth(); viewDate = new Date(n);
        setSelectedDate(n.toISOString().slice(0, 10)); renderCalendar(); renderSlotsList();
      });

      function setView(v) {
        calView = v;
        ['#view-month', '#view-week', '#view-day'].forEach(sel => {
          const b = qs(sel);
          if (b) { const on = (sel === '#view-' + v); b.setAttribute('aria-pressed', on ? 'true' : 'false'); }
        });
        renderCalendar();
      }
      ...
      const selDate = qs('#selected-date');
      if (selDate) selDate.value = new Date().toISOString().slice(0, 10);
      renderSlotsList();

      // resume params: auto-open booking modal
      try {
    ...
    async function renderMatch(providerId) {
      try {
        ...
        // computeMatchForProvider is a legacy module; skip silently if unavailable
  ...
  return (
    <main className="section">
      <div className="container sched-wrap stack">
        <h1 className="section-title" id="title">スケジュール選択</h1>
        <p className="muted" id="subtitle"></p>
        <div id="match-host" className="card" style={{ padding: '12px' }} hidden></div>
        <div className="card" style={{ padding: '16px' }}>
          <div className="cluster" style={{ justifyContent: 'space-between', alignItems: 'center' }}>
            <div className="cluster" style={{ gap: '8px', alignItems: 'center' }}>
              <button id="prev-month" className="btn btn-ghost" type="button">← 前月</button>
              <strong id="cal-title"></strong>
              <button id="next-month" className="btn btn-ghost" type="button">次月 →</button>
            </div>
            <div className="cluster" style={{ gap: '8px', alignItems: 'center', flexWrap: 'wrap' }}>
              ...
              <button id="today-btn" className="btn btn-ghost" type="button">今日</button>
            </div>
          </div>
          <div id="calendar" className="stack" style={{ marginTop: '12px' }}></div>
        </div>
        <div className="card" style={{ padding: '16px' }}>
          <div className="cluster" style={{ justifyContent: 'space-between', alignItems: 'center' }}>
            <div className="cluster" style={{ gap: '8px', alignItems: 'center' }}>
            ...
            <div className="cluster" style={{ gap: '8px', alignItems: 'center' }}>
              <label>サービス
                <select id="service-select" style={{ minWidth: '240px' }}>
                  <option value="">未選択（選択してください）</option>
                </select>
              </label>
            </div>
          </div>
          <div id="slots-list" className="stack" style={{ marginTop: '12px', gap: '8px' }}></div>
          <div style={{ marginTop: '8px' }}>
            <button id="request-other" className="btn btn-ghost" type="button">別の時間をリクエストする</button>
          </div>
        </div>

        {/* Modal: Booking Request */}
        <div id="req-modal-backdrop" className="modal-backdrop" hidden></div>
        <div id="req-modal" className="modal" role="dialog" aria-modal="true" aria-labelledby="req-modal-title" hidden>
          <div className="modal-content card" style={{ padding: '20px' }}>
            <div className="cluster" style={{ justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
              ...
              <button id="req-modal-close" className="btn btn-ghost" type="button" aria-label="閉じる">×</button>
            </div>
            <form id="req-form" className="stack" style={{ gap: '12px' }}>
              <input type="hidden" id="req-serviceId" />
              <input type="hidden" id="req-slotId" />
              <div className="stack">
                <div id="req-slot-summary" className="muted"></div>
                ...
                <div className="cluster">
                  <button id="req-submit" className="btn" type="submit">送信</button>
                  <button id="req-cancel" className="btn btn-ghost" type="button">キャンセル</button>
                  <span id="req-msg" className="muted" aria-live="polite"></span>
                </div>
              </div>
              {/* Modal: Auth prompt for unauthenticated users */}
              <div id="auth-prompt-backdrop" className="modal-backdrop" hidden style={{ zIndex: 11000 }}></div>
              <div id="auth-prompt-modal" className="modal" role="dialog" aria-modal="true" aria-labelledby="auth-prompt-title" hidden style={{ zIndex: 11001 }}>
                <div className="modal-content card" style={{ padding: '20px', width: 'min(100%,440px)' }}>
                  <div className="cluster" style={{ justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                    <h2 id="auth-prompt-title" style={{ margin: '0' }}>ログインまたは新規登録してください</h2>
                    <button id="auth-prompt-close" className="btn btn-ghost" type="button" aria-label="閉じる">×</button>
                  </div>
                  <p className="muted">予約を行うにはログインが必要です。続行するにはログインまたは新規登録をしてください。</p>
                  <div className="cluster" style={{ justifyContent: 'flex-end', marginTop: '12px', gap: '8px' }}>
                    <a id="auth-prompt-register" className="btn btn-ghost" href="/user/login">新規登録</a>
                    <a id="auth-prompt-login" className="btn" href="/user/login">ログイン</a>
                  </div>
                </div>
              </div>
            </form>
