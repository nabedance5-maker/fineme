...
export default function BelleDiagnosisResultPage() {
  ...
  useEffect(() => {
    ...
    const TYPE_DESCRIPTION = {
      ...
      BAV:'自分でトレーニングや食事管理をしている。その自立心は強みだ。でもパーソナルトレーナーやプロの栄養士に頼ったことがない。独学には限界があり、特に「自分に合ったやり方」を見つけるのが難しい。一度だけプロに相談してみよう。今の取り組みの何が正しくて、何が足りないかが明確になる。そこからは自分でも精度高く動ける。',
      ...
      EPD:'眉サロンに通いながら、今まさにケアを続けている。外見の中で最も即効性の高い軸を、プロとともに磨いている。この状態で次に目を向けるべきは、整った眉を「全体の印象設計」の一部として活かすことだ。眉の形に合わせた髪型・肌・服装との調和を意識すると、全体の印象がさらに引き上がる。',
      // ── F軸（服装・ファッション） ──
      ...
      RCD:'脱毛に関心を持ち、今まさに動き始めている。この段階で一番大事なのは「部位の優先順位」だ。すべてを一度にやろうとすると費用と期間がかさむ。ヒゲ・腕・脚など、見えやすい部位から優先するとコスパよく進められる。カウンセリングで優先部位を相談しよう。',
    ...
    const AXIS_WORD = { B:'鋼の', E:'眉弧の', F:'纏いの', H:'黒髪の', S:'光肌の', R:'素肌の', T:'白砂の', W:'翡翠の' };
    const TYPE_MODIFIER = {
      NV:'眠れる', NK:'知らぬ', ND:'臥す',
    ...
    const html = `
      ...
      ${compassFirstDef.label ? `<div style="margin:12px 0 20px;padding:16px 18px;background:rgba(10,15,30,0.65);border:1px solid rgba(201,168,76,0.28);border-radius:12px;display:flex;align-items:center;gap:14px;backdrop-filter:blur(8px)">
        ...
        <a href="/belle/mirror" style="font-size:12px;font-weight:800;padding:10px 14px;background:rgba(200,100,140,0.1);border:1.5px solid rgba(200,100,140,0.7);color:rgba(200,100,140,0.9);border-radius:8px;text-decoration:none;white-space:nowrap;flex-shrink:0;text-align:center;line-height:1.4">Mirror<br><span style="font-size:10px;font-weight:600">¥500</span></a>
      </div>` : ''}
      ...
      <div class="save-map-cta">
        ...
        <div class="save-map-cta-body">
          ...
          <a href="/login?mode=signup&next=/belle/diagnosis/result" class="save-map-cta-btn">無料アカウントを作って保存する →</a>
