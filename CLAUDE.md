...
### 4. 機能実装・仕様決定をしたら、言われなくても事業ロードマップを更新する

`public/business/fineme-roadmap.html` は事業の現在地を一目で見せるための地図。週次cron（`~/fineme/roadmap-agent.md`）は更新し忘れの保険であって唯一の更新経路ではない。

新機能の実装・重要な仕様決定（価格変更、上限設定、訴求文言の変更等）をコミット・プッシュしたら、**でおに聞かれる前に**同じセッション内でロードマップのNOW BOARD・該当Phase詳細も更新し、コミット・プッシュする。「ロードマップ更新して」と言われるのを待たない。
- 更新範囲・書き方のルールは `roadmap-agent.md` の「更新範囲」「運用方針」に従う（CSS・Phase 0/3/4本体・確定済み過去ログは変更しない）
- 些細な文言修正・タイポ修正など地図として意味を持たない変更は対象外。事業の進捗・意思決定として記録に値するものが対象
